...
from hydra_plugins.hydra_dh_launcher.dh_launcher import DHLauncher
...
def test_discovery() -> None:
    ...
    assert DHLauncher.__name__ in [
        x.__name__ for x in Plugins.instance().discover(Launcher)
    ]


@mark.parametrize("launcher_name, overrides", [("dh", [])])
class TestDHLauncher(LauncherTestSuite):
    """
...
@mark.parametrize(
    ...
    [
        # dh with process-based backend (default)
        (
            {},
            [
                "-m",
...
class TestDHLauncherIntegration(IntegrationTestSuite):
    """
